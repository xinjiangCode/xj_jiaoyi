	...
	smallimgClick: function() {
		...
		$("." + that.smallimg).click(function() {
			$("." + that.bigimg).css({
				...
				height: $("." + that.smallimg).height() * 5,
				width: $("." + that.smallimg).width() * 1.5
			});
			$("." + that.mask).fadeIn();
			// $("." + that.bigimg).attr("src", $(this).attr("src")).fadeIn()
			$("." + that.bigimg).attr("src", $(this).parent().find('input[type="hidden"]').val()).fadeIn()

			// $("." + that.bigimg).css("background", $(this).parent().find('input[type="hidden"]').val()).fadeIn()
